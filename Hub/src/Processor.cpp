...
namespace {
    ...
    void GetTemperatures(SendReply sendFunction) {
        String result;

        const std::set<Mac> thermometers = ListThermometers();
        for (const Mac& thermometer : thermometers) {
            const String name = GetName(thermometer);
            const float temperature = GetThermometerValue(thermometer);
    ...
    void GetRelayStates(SendReply sendFunction) {
        String result;

        const std::set<Mac> relays = ListRelays();
        for (const Mac& relay : relays) {
            const String name = GetName(relay);
            const bool state = GetRelayState(relay);
            result += name + ":" + (state ? "on" : "false") + "\n";
        }

        const std::set<Mac> thermostatRelays = ListThermostatRelays();
        for (const Mac& thermostatRelay : thermostatRelays) {
            const String name = GetName(thermostatRelay);
            const RelayState relayState = GetThermostatRelayState(thermostatRelay);
            const bool state = relayState.manualMode ? relayState.manualState : relayState.temperatureState;
            result += name + ":" + (state ? "on" : "false") + "\n";
        }
...
void ProcessCommand(const String& command, SendReply sendFunction) {
    ...
    for (const String& oneCommand : commands) {
        ...
        } else if (oneCommand.startsWith("SET_RELAY_TEMPERATURE_DELTA;")) {
            // SET_RELAY_TEMPERATURE_DELTA;00:00:00:00:00:00;value$
            // SET_RELAY_TEMPERATURE_DELTA;name;value$
            // SET_RELAY_TEMPERATURE_DELTA;ALL;value$
            SetRelayTemperatureDelta(oneCommand);
        } else if (oneCommand == "GET_TEMPERATURES$") {
            // GET_TEMPERATURES$
            GetTemperatures(sendFunction);
        } else if (oneCommand == "GET_RELAY_STATES$") {
            // GET_RELAY_STATES$
            GetRelayStates(sendFunction);
        }
    }
}
